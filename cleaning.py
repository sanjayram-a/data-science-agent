...
def clean_data(df: pd.DataFrame, options: Optional[Dict[str, Any]] = None) -> Tuple[Optional[pd.DataFrame], List[str]]:
    ...
    print("--- Cleaning Data ---")
    print("Options used:", options) # Show options being used
    ...
    if missing_method != "none":
        ...
        if total_missing_before > 0:
            ...
            if total_missing_before > 0 and total_missing_after == 0:
                 print("✔️ All missing values handled.")
            elif total_missing_after > 0:
                 print(f"⚠️ Still {total_missing_after} missing values remaining after handling.")
    ...
    if not cleaning_steps:
        ...
        print("No cleaning steps were applied based on the provided options.")
